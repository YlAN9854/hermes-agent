...
> 方向 A 阶段 1+2(标记+预览)/ **阶段 3 应用 drop + A-v2 fold** / **压缩闸门第一阶段 + 第二阶段 drop 编辑首刀** /
...
> **死重清单主动清理(系统建议折已完成支线 → 复用应用 fold)** + **闸门内 drop 编辑(被迫压缩前先删垃圾,经应答落地、两暗礁消解)** +
> **按 turn 看时间/主题、点轮跳对话、整轮主动 fold/drop 的语义 minimap / 主操作台(band 概览 + inspector 类型细节)** 均闭环。
> **下一步:复用闸门"应答带回编辑"keystone 做闸门内 fold/keep + 死重喂闸门 + 纠正 focus;R 滚动增量 / ②→①完成自动触发;
