...
| [v2/operations.md](v2/operations.md) | 操作集(动词轴 + 放置轴)+ 交互负担(二阶剃刀)+ 引用图(弧叠加/画布/产物索引)+ 新交互 + 驱动 case | 设计中;**引用图层①②+画布化(TurnCanvas,弧落子格)+产物/关键词索引追踪 + `add` 首刀(fixture 工坊+预览+pin+inspector)已建成**;add 落地(Layer 2)待建 |
