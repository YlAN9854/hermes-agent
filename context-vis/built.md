...
- **② 压缩后不回退 = 显示放宽 + 结构变更重取**(踩坑修正):**根因不是 colorOn,是 `colorsFresh` 把显示钉死在精确 hv 相等**,而压缩后 hv **二次跳动**(`emit_post_compaction_snapshot` 不带 hv → 着色暂活;本轮末常规快照才把 hv 推到 N+1 → 失效回中性),"按 `compressionCount` 重取"只抓到第一拍 → 永久中性。
  ...
  - **②补 · 重测着色"进行中"提示(用户复测提出)**:压缩应用后 turn 即更新,但结构重取要跑后台辅助模型数秒、期间仍显旧色 → 用户误以为已是最终态。
    加 **render 派生 `colorStale`**(= 与结构重取 effect **同判据**:此前着色的某 chunkId 在当前 chunks 消失;**纯派生、无 effect 置态 → 避开 `set-state-in-effect`**,这正是原 fetch 刻意回避的)+ `coloringPending = wantColor && (colorBusy || colorStale)` → band 上方脉冲提示「主题已变,正在重测着色…」/「主题检测中…」,重取完成 colorData 换新 id → 自动消失。**不改同步性**(重测本质要 LLM,强同步会卡住已正常的 turn 更新),如实告诉用户"turn 已新、颜色在追上来"=白盒。
