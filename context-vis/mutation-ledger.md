# 结构突变账本（mutation ledger）—— 让轮次拓扑扛过压缩/折叠/删除

> **状态:设计定稿(本次讨论定型),未动工。** 修法 ②。turn 带([turn-band.md](turn-band.md))
> 致命弱点:**它跑在被压坏的历史上**——压缩/fold/drop 销毁真实轮、注入摘要块,band 靠数当前
> history 切轮,只能看到残骸。本件给出根治:**在任何结构突变前留痕,把折叠出处挂回折叠块**,
> 让 band 还原"有过哪些轮、谁被折了"。相关:[turn-band.md](turn-band.md) §4、[regime.md](regime.md)
> "跑在压缩后历史/压缩前快照"、[fold.md](fold.md)、[compaction-gate.md](compaction-gate.md)。

> **一句话**:上下文的轮次拓扑会被三种突变损毁(自动压缩 / 手动 fold / 手动 drop)。
