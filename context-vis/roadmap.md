...
3. ⬜ **应用(真正动上下文)**:把 fate map 下发,**驱动既有
   ...
   可由 sub-agent 执行。**← 下一步,卡在下方命令通道硬骨头。**
...
- **A. 交互式压缩(旗舰)**:①选中+fate 标记 + ②预览释放量 **已建成**(纯前端零风险)。
  **剩 ③应用**:把 fateMap 下发驱动 `compress()` 真正动上下文——必须先解"浏览器→真实
  PTY 会话"命令通道(见上文硬骨头)。这是全项目下一个真正的架构决策点。
...
> 建议优先级:**A③ 命令通道**(产品定位的核心,已是唯一拦路点)> B/C2(增量) > D(收尾)。
> A 的 ①② 已落地出手感,下一步就是攻 ③ 的"浏览器→真实 PTY 会话"通道。
