...
4. ⬜ **后续(未做)**:深析**死重清单**(喂闸门 + 方向 A 主动清理)、②→① 完成检测触发清理、
   滚动增量(压缩前快照,根治"跑在压缩后历史")、产品门槛(纯浏览算不算任务)、非闸门路喂 focus、闸门内编辑 focus。
5. ⬜ **持久自动主题着色(turn 带常驻底色)** — 用户需求(2026-06-16 调研定调,本次只记录不动工)。
...
- **R. 任务态识别(自适应总开关)**:**第一刀(启发式)+ 第二刀(LLM 语义)均已建成并实测**——
  ...
  **后续①焦点压缩(focus → `focus_topic`,仅闸门路)✅ 已建成实测**。**剩**:深析死重清单(喂闸门 +
  方向 A 主动清理)、滚动增量、产品门槛、非闸门路喂 focus、闸门内编辑 focus。
