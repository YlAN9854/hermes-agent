...
### 任务态识别(ContextVis 的自适应总开关,下一旗舰)
**多轮需求讨论的中枢结论**,设计见 [regime.md](regime.md)。ContextVis 不该常驻打扰——
应在**位置式压缩将要背叛你的任务时才启动**:森林态(turns 独立)休眠、像普通 Hermes,
主线态(长期目标)才介入。一个**常驻 agent 侧检测器**(廉价嗅探 → 按需深析)给出判断,
驱动**两级闸门**(regime ∧ collision)+ **焦点压缩**(检测焦点喂 `focus_topic`)+ **死重清单**
(喂闸门 + 方向 A 主动清理)。动作原语(fold/drop/闸门)已建成,缺的是这个脑 + 门控逻辑。
1. ⬜ **第一刀:廉价嗅探 + 两级门控** — 粗信号判森林/任务,让闸门"森林里闭嘴、任务里出声"。
2. ⬜ **第二刀:深析死重清单** — aux 模型产焦点 + 排序清单,喂闸门与主动清理。
3. ⬜ **后续**:②→① 完成检测触发清理、轮内拆分、交错支线归并。
...
- **R. 任务态识别(下一旗舰)**:常驻检测器 + 两级闸门 + 焦点压缩 + 死重清单,见
  [regime.md](regime.md) 与 [needs.md](needs.md) §E E0。**第一刀 = 廉价嗅探 + 两级门控**
  (森林闭嘴、任务出声),第二刀 = 深析死重清单。吸收并重定向了原 E3/B(不再追求全局语义
  聚类),复用已建成的 fold/drop/闸门。
- **G. 压缩闸门**:第一阶段(预览 + 确认)**已建成**。后续与 R 合流:闸门从"逢阈值弹"走向
  "**regime ∧ collision 才弹** + 闸门内确认/纠正压缩焦点"。原"第二阶段=闸门内编辑"仍可叠加。
- **E4. treemap 语义排列模式**(降优,可选):「时间序 ⇄ 语义聚类」模式切换,让同语义块聚拢好框选。
  **红线:模式切换而非替换**(时间序保底,CLAUDE.md 第 5 条)。R 路核心价值在闸门里就兑现,
  此独立视图非必需 → 降为可选打磨。
...
> 建议优先级:**R 任务态识别第一刀**(廉价嗅探 + 两级门控,让闸门"森林闭嘴、任务出声",
> 立刻验证自适应命题,见 [regime.md](regime.md))> R 第二刀(深析死重清单)> C2(增量)>
> D/E4(收尾/可选)。
> A 的 ①②③(drop+fold)与 G 闸门一阶段已闭环出手感;下一步是给它们装"脑"——
> 一个判森林/任务、决定何时该醒的常驻检测器。
