...
6. ◑ **持久主题着色(同主题同色)— 已建成(代码+构建通过,待 dashboard 实测)**。用户需求(2026-06-16 提出)。
   **痛点收缩(用户 2026-06-18 敲定)**:不要求"每轮自动检测/常驻",**只要求**「主题着色」按钮 / 闸门着色时
   **同一主题跨多次着色保持同色**(例:"opencode 源码学习"首次粉 → 此后一直粉)。
   **根因**:`buildTopicColorMap` 按 topic 串**排序后的下标**分色 + 每次 render 从零重建无记忆 → 主题集合一变即**整排重洗**。
   **已定方案 = 前端持久「主题→色」注册表(`assignTopicColors`)**:首见某主题分下一空闲色并记下、以后复用、**只增不洗**;
   存 **localStorage 按 sessionId**(浮层折叠会重挂载 panel,内存 ref 会丢);**归一化键**(trim/小写/压空格)吸收 LLM 琐碎改名。
   **LLM 着色与检测频率不变、零后端/Hermes 改动**。残留:LLM **显著**改名仍可能跳色 → 留白(实测确有再加保守模糊匹配)。
   - ✗ **否决:启发式连通分量聚类着色**(用户经验上效果差,放弃以启发式打底)。
   - ✗ **否决:LLM 增量着色 / 按 turn 内容 hash 缓存 / 只判新轮**——用户**不要求**每轮检测,无需此基建(原"与滚动增量同机器"的动机随之消失)。
   - ✗ **不做朴素"每轮全量 LLM"**:成本高 + 抖动。
